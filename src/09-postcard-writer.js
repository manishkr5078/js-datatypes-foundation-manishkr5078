...
export function writePostcard(sender, receiver, message) {
  ...
  const newSender = sender.trim();
  const newReceiver = receiver.trim();
  const newMessage = message.trim();

  if(newSender.length === 0 || newReceiver.length === 0 || newMessage.length === 0)
    return "";
...
export function formatPostcardField(label, value, width) {
  ...
  if(typeof width !== "number" || width === null || width === undefined)
    width = 12;
